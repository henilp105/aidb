...
def _get_estimate_bennett(
    estimate: float, std: float, max_statistic: float, num_samples: int, conf: float
) -> Estimate:
    delta_inp = 1.0 - conf
        ...
        * (
            np.exp(
                scipy.special.lambertw(
                    (a**2 * np.log(1.0 / delta) - var_ub) / (math.e * var_ub)
                )
                + 1
            )
            - 1
...
class Estimator(abc.ABC):
    ...
    @abc.abstractmethod
    def estimate(
        self, samples: pd.DataFrame, num_samples: int, conf: float, **kwargs
    ) -> Estimate:
        pass
...
class WeightedMeanSetEstimator(Estimator):
    def estimate(
        ...
        return _get_estimate_bennett(
            wstats.mean, wstats.std, np.abs(cstats).max(), len(cstats), conf
        )
...
class WeightedCountSetEstimator(Estimator):
    def estimate(
        ...
        statistics = np.array(
            samples.iloc[:, 0].tolist() + [0.0] * (num_samples - len(samples))
        )
        wstats = DescrStatsW(statistics, weights=weights, ddof=0)
        mean_est = _get_estimate_bennett(
            wstats.mean, wstats.std, np.abs(statistics).max(), len(statistics), conf
        )
