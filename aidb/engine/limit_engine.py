...
class LimitEngine(TastiEngine):
    ...
    async def _execute_limit_query(self, query: Query):
        ...
        id_score = [
            (i, s)
            for i, s in zip(
                proxy_score_for_all_blobs.index, proxy_score_for_all_blobs.values
            )
        ]
        ...
        for bound_service in bound_service_list:
            limit_engine_batch_size = min(
                bound_service.service.preferred_batch_size, limit_engine_batch_size
            )
        ...
        for batched_indexes in batched_indexes_list:
            for bound_service in bound_service_list:
                inp_query_str = (
                    self.get_input_query_for_inference_service_filtered_index(
                        bound_service, self.blob_mapping_table_name, batched_indexes
                    )
                )
