...
class ApproximateAggregateEngine(FullScanEngine):
    async def execute_aggregate_query(self, query: Query):
        ...
        inference_engines_required = (
            query.inference_engines_required_for_filtering_predicates
        )
        for filtering_predicate, engine_required in zip(
            filtering_predicates, inference_engines_required
        ):
            if len(engine_required) == 0:
                blob_key_filtering_predicates.append(filtering_predicate)
        blob_key_filtering_predicates_str = self._get_where_str(
            blob_key_filtering_predicates
        )
        blob_key_filtering_predicates_str = (
            f"WHERE {blob_key_filtering_predicates_str}"
            if blob_key_filtering_predicates_str
            else ""
        )

        async with self._sql_engine.begin() as conn:
            blob_count_query_str = self.get_blob_count_query(
                blob_tables, blob_key_filtering_predicates_str
            )
            ...
            alpha = (1.0 - (query.confidence / 100.0)) / num_aggregations
            conf = 1.0 - alpha

            num_samples = self.get_additional_required_num_samples(
                query, sample_results, alpha
            )
            ...
            new_sample_results = await self.get_results_on_sampled_data(
                num_samples, query, blob_tables, blob_key_filtering_predicates_str, conn
            )

        concatenated_results = pd.concat(
            [sample_results, new_sample_results], ignore_index=True
        )
        # TODO:  figure out what should parameter num_samples be for COUNT/SUM query

        estimates = []
        fixed_cols = concatenated_results[
            [NUM_ITEMS_COL_NAME, WEIGHT_COL_NAME, MASS_COL_NAME]
        ]
        for index, (agg_type, _) in enumerate(aggregation_type_list):
            selected_index_col = concatenated_results.iloc[:, [index]]
            extracted_sample_results = pd.concat(
                [selected_index_col, fixed_cols], axis=1
            )

            estimator = self._get_estimator(agg_type)
            estimates.append(
                estimator.estimate(
                    extracted_sample_results, _NUM_PILOT_SAMPLES + num_samples, conf
                ).estimate
            )
    ...
    def get_blob_count_query(
        self, table_names: List[str], blob_key_filtering_predicates_str: str
    ):
    ...
    def _get_estimator(self, agg_type: exp.Expression) -> Estimator:
        if agg_type == exp.Avg:
            ...
            raise NotImplementedError(
                "We only support AVG, COUNT, and SUM for approximate aggregations."
            )

    def get_sample_blobs_query(
        self,
    ...
    async def execute_inference_services(
        self, query: Query, sample_df: pd.DataFrame, conn
    ):
        """
        ...
        for bound_service in bound_service_list:
            inp_query_str = self.get_input_query_for_inference_service_filter_service(
                bound_service, query, inference_services_executed
            )
            new_query = Query(inp_query_str, self._config)
            query_add_filter_key, _ = self.add_filter_key_into_query(
                list(bound_service.binding.input_columns), sample_df, new_query
            )

            input_df = await conn.run_sync(
                lambda conn: pd.read_sql_query(text(query_add_filter_key.sql_str), conn)
            )
            await bound_service.infer(input_df)
            inference_services_executed.add(bound_service.service.name)

    async def get_results_for_each_blob(
        self, query: Query, sample_df: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        table_columns = [
            ...
            for col in self._config.tables[table].columns
        ]
        query_add_filter_key, selected_column = self.add_filter_key_into_query(
            table_columns, sample_df, query_no_aqp
        )
        query_add_count = query_add_filter_key.add_select(
            f"COUNT(*) AS {NUM_ITEMS_COL_NAME}"
        )
        query_str = f"""
        ...
        async with self._sql_engine.begin() as conn:
            res_df = await conn.run_sync(
                lambda conn: pd.read_sql_query(text(query_str), conn)
            )
        res_df[WEIGHT_COL_NAME] = 1.0 / self.blob_count
    ...
    async def get_results_on_sampled_data(
        ...
        sample_blobs_query_str = self.get_sample_blobs_query(
            blob_tables, num_samples, blob_key_filtering_predicates_str
        )
        sample_blobs_df = await conn.run_sync(
            lambda conn: pd.read_sql_query(text(sample_blobs_query_str), conn)
        )

        if len(sample_blobs_df) < num_samples:
            raise Exception(
                f"Require {num_samples} samples, but only get {len(sample_blobs_df)} samples"
            )

        await self.execute_inference_services(query, sample_blobs_df, conn)

        results_for_each_blob = await self.get_results_for_each_blob(
            query, sample_blobs_df
        )
    ...
    def _calculate_required_num_samples(
        ...
        if agg_type == exp.Avg:
            p_lb = statsmodels.stats.proportion.proportion_confint(
                len(extracted_sample_results), sample_size, alpha
            )[0]
        else:
            p_lb = 1
        ...
        return int(
            (
                ...
                * pilot_estimate.std_ub
                / (error_target * pilot_estimate.lower_bound)
            )
            ** 2
            * (1.0 / p_lb)
        )
    ...
    def get_additional_required_num_samples(
        self, query: Query, sample_results: pd.DataFrame, alpha
    ) -> int:
        error_target = query.error_target
        num_samples = []

        fixed_cols = sample_results[
            [NUM_ITEMS_COL_NAME, WEIGHT_COL_NAME, MASS_COL_NAME]
        ]
