...
from aidb.utils.constants import (
    table_name_for_rep_and_topk_and_blob_mapping,
    VECTOR_ID_COLUMN,
)
...
class TastiEngine(FullScanEngine):
    ...
    async def get_proxy_scores_for_all_blobs(self, query: Query, **kwargs):
        ...
        if (
            self.rep_table_name not in self._config.tables
            or self.topk_table_name not in self._config.tables
        ):
        ...
        for bound_service in bound_service_list:
            inp_query_str = self.get_input_query_for_inference_service_filtered_index(
                bound_service, self.rep_table_name
            )
        ...
        score_query_str, score_connected = self.get_score_query_str(
            query, self.rep_table_name
        )
        async with self._sql_engine.begin() as conn:
            score_df = await conn.run_sync(
                lambda conn: pd.read_sql_query(text(score_query_str), conn)
            )
    ...
    def _get_filter_predicate_score_map(
        self, query: Query
    ...
    def get_score_query_str(
        self, query: Query, rep_table_name: str
    ) -> (str, List[List[str]]):
        """
        Convert filtering condition into select clause, so we can use select result to compute proxy score for all blobs
        """
        filering_predicate_score_map, score_connected = (
            self._get_filter_predicate_score_map(query)
        )
        score_list = []

        for fp in filering_predicate_score_map:
            score_list.append(
                f"(CASE WHEN {fp} THEN 1 ELSE 0 END) AS {filering_predicate_score_map[fp]}"
            )
    ...
    async def propagate_score_for_all_vector_ids(
        self, score_df: pd.DataFrame, return_binary_score=False
    ) -> pd.DataFrame:
        topk_query_str = f"SELECT * FROM {self.topk_table_name}"
        async with self._sql_engine.begin() as conn:
            topk_df = await conn.run_sync(
                lambda conn: pd.read_sql_query(text(topk_query_str), conn)
            )
    ...
    def _create_tasti_table(self, topk: int, conn: sqlalchemy.engine.base.Connection):
        ...
        blob_mapping_table = sqlalchemy.schema.Table(
            self.blob_mapping_table_name, metadata, autoload=True, autoload_with=conn
        )

        # FIXME: there is a sqlalchemy SAWarning, This warning may become an exception in a future release
        rep_table = sqlalchemy.schema.Table(
            self.rep_table_name,
            metadata,
        ...
        for i in range(topk):
            ...
            columns.append(
                sqlalchemy.Column(f"topk_dists_{str(i)}", sqlalchemy.Float(32))
            )
    ...
    def _format_topk_for_all(self, topk_for_all: pd.DataFrame) -> pd.DataFrame:
        ...
        new_topk_for_all = {
            f"topk_reps_{i}": topk_for_all["topk_reps"].str[i] for i in range(topk)
        }
        new_topk_for_all.update(
            {f"topk_dists_{i}": topk_for_all["topk_dists"].str[i] for i in range(topk)}
        )
        new_topk_for_all.update(
    ...
    async def initialize_tasti(self):
        ...
        if self.user_specified_vector_ids is not None:
            ...
            async with self._sql_engine.begin() as conn:
                vector_ids = await conn.run_sync(
                    lambda conn: pd.read_sql_query(
                        text(vector_id_select_query_str), conn
                    )
                )
        ...
        async with self._sql_engine.begin() as conn:
            ...
            rep_blob_df = await conn.run_sync(
                lambda conn: pd.read_sql_query(text(rep_blob_query_str), conn)
            )

            await conn.run_sync(
                lambda conn: rep_blob_df.to_sql(
                    self.rep_table_name, conn, if_exists="append", index=False
                )
            )
            await conn.run_sync(
                lambda conn: new_topk_for_all.to_sql(
                    self.topk_table_name, conn, if_exists="append", index=False
                )
            )
