...
class ApproximateAggregateJoinEngine(ApproximateAggregateEngine):
    async def get_rows_in_join_tables(
        self, blob_tables: List[str], blob_key_filtering_predicates
    ):
        """
        ...
        for table in blob_tables:
            ...
            for fp in blob_key_filtering_predicates:
                if len(fp) > 1:
                    raise Exception(
                        "OR operator for blob keys filtering is not supported."
                    )
    ...
    def execute_join_query_sampling(
        self, sample_df_list, num_samples, col_to_alias, agg_col_to_alias
    ):
        ...
        sample_res_df = sampled_cols[
            list(col_to_alias.keys()) + list(agg_col_to_alias.keys())
        ]
        sample_res_df.columns = list(col_to_alias.values()) + list(
            agg_col_to_alias.values()
        )
        sample_res_df = sample_res_df.copy()
        sample_res_df[WEIGHT_COL_NAME] = 1.0 / self.blob_count
    ...
    async def execute_inference_services_join(
        ...
        res_satisfy_udf = self.execute_user_defined_function(
            sample_df, dataframe_sql, query, blob_keys + fixed_cols + agg_cols
        )
    ...
    async def execute_sampling_and_join_services(
        ...
        inference_engines_required = (
            udf_query.inference_engines_required_for_filtering_predicates
        )
        ...
        sample_df_list = await self.get_rows_in_join_tables(
            blob_tables, blob_key_filtering_predicates
        )
        ...
        while total_sampled_count < num_samples:
            sample_res_df = self.execute_join_query_sampling(
                sample_df_list, RETRIEVAL_BATCH_SIZE, col_to_alias, agg_col_to_alias
            )
            ...
            agg_res_list = await self.execute_inference_services_join(
                query, sample_res_df, blob_keys, agg_list
            )
    ...
    def get_additional_required_num_samples_join(
        ...
        while index < len(agg_type_with_cols):
            ...
            extracted_sample_results = sample_result[
                [agg_col, NUM_ITEMS_COL_NAME, WEIGHT_COL_NAME, MASS_COL_NAME]
            ]
            num_samples.append(
    ...
    async def execute_aggregate_join_query(self, query: Query):
        ...
        agg_type_with_cols = (
            query_no_aqp.query_after_normalizing.aggregation_type_list_in_query
        )
        ...
        for res in res_list:
            if len(res) == 0:
                raise Exception(
                    "No matched pair found. Please use a larger oracle budget."
                )

        num_aggregations = len(agg_list)
        alpha = (1.0 - (query.confidence / 100.0)) / num_aggregations
        conf = 1.0 - alpha
        num_samples = self.get_additional_required_num_samples_join(
            query, res_list, agg_list, alpha
        )
        ...
        if len(new_res_list) != 0:
            for res, new_res in zip(res_list, new_res_list):
                concatenated_results.append(
                    pd.concat([res, new_res], ignore_index=True)
                )
        else:
        ...
        while index < len(agg_type_with_cols):
            ...
            estimates.append(
                estimator.estimate(
                    sample_res, _NUM_PILOT_SAMPLES + num_samples, conf
                ).estimate
            )
            index += 1
