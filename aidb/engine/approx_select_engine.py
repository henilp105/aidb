...
class ApproxSelectEngine(TastiEngine):
    ...
    def score_fn(
        self, score_for_all_df: pd.DataFrame, score_connected: List[List[str]]
    ) -> pd.Series:
        """
    ...
    async def get_inference_results(self, query: Query, sampled_index: List[int], conn):
        ...
        for bound_service in bound_service_list:
            inp_query_str = self.get_input_query_for_inference_service_filtered_index(
                bound_service, self.blob_mapping_table_name, sampled_index
            )
            inp_df = await conn.run_sync(
                lambda conn: pd.read_sql_query(text(inp_query_str), conn)
            )
            ...
            for col in self._config.tables[table_name].columns:
                ...
                if col_name in blob_keys and col_name not in added_cols:
                    join_conditions.append(
                        f"{self.blob_mapping_table_name}.{col_name} = {table_name}.{col_name}"
                    )
        ...
        all_query_add_filter_key, _ = self.add_filter_key_into_query(
            table_columns, sampled_df, query_no_where
        )

        async with self._sql_engine.begin() as conn:
            all_df = await conn.run_sync(
                lambda conn: pd.read_sql_query(
                    text(all_query_add_filter_key.sql_str), conn
                )
            )
            ...
            sample_query_add_filter_key, _ = self.add_filter_key_into_query(
                table_columns, sampled_df, query_after_adding_join
            )

            res_df = await conn.run_sync(
                lambda conn: pd.read_sql_query(
                    text(sample_query_add_filter_key.sql_str), conn
                )
            )
    ...
    def get_sampled_proxy_blob(
        self, proxy_score_for_all_blobs, defensive_mixing: int = 0.1
    ):
        weights = proxy_score_for_all_blobs.values**0.5
        normalized_weights = (1 - defensive_mixing) * (
            weights / sum(weights)
        ) + defensive_mixing / len(weights)
    ...
    def tau_modified(
        self, satisfied_sampled_results, recall_target, confidence, total_length
    ):
        z = satisfied_sampled_results[MASS_COL_NAME]
        estimated_tau = self.tau_estimate(
            recall_target * sum(z), satisfied_sampled_results
        )
        positive_length = len(
            satisfied_sampled_results[
                satisfied_sampled_results[PROXY_SCORE_COL_NAME] >= estimated_tau
            ]
        )

        estimated_z1 = list(z[:positive_length]) + [0] * (
            total_length - len(z[:positive_length])
        )
        estimated_z2 = list(z[positive_length:]) + [0] * (
            total_length - len(z[positive_length:])
        )
        ...
        _, ub = self._get_confidence_bounds(
            z1_mean, z1_std, len(estimated_z1), delta / 2
        )
        lb, _ = self._get_confidence_bounds(
            z2_mean, z2_std, len(estimated_z2), delta / 2
        )
        ...
        modified_tau = self.tau_estimate(
            modified_recall_target * sum(z), satisfied_sampled_results
        )
    ...
    async def execute_approx_select_query(self, query: Query, **kwargs):
        if not query.is_valid_approx_select_query:
            raise Exception(
                "Approx select query should contain Confidence and should not contain Budget."
            )

        # generate proxy score for each blob
        score_for_all_df, score_connected = await self.get_proxy_scores_for_all_blobs(
            query
        )
        ...
        if SEED_PARAMETER in kwargs:
            ...
            seed = None
        sampled_df = dataset.sample(
            BUDGET, replace=True, weights=WEIGHT_COL_NAME, random_state=seed
        )

        async with self._sql_engine.begin() as conn:
            satisfied_sampled_results, all_sampled_results = (
                await self.get_inference_results(query, list(sampled_df.index), conn)
            )

            satisfied_sampled_results = satisfied_sampled_results.join(
                sampled_df, how="inner"
            )
            sorted_satisfied_sampled_results = satisfied_sampled_results.sort_values(
                by=PROXY_SCORE_COL_NAME, ascending=False
            )

            no_result_length = BUDGET - len(
                sampled_df.loc[list(set(all_sampled_results.index))]
            )
            ...
            additional_sample_index = dataset[
                dataset[PROXY_SCORE_COL_NAME] >= tau_modified
            ].index

            all_selected_blob = list(
                set(pilot_sample_index).union(set(additional_sample_index))
            )

            logger.info(f"num_samples: {len(all_selected_blob)}")

            all_satisfied_sampled_results, _ = await self.get_inference_results(
                query, all_selected_blob, conn
            )
