...
def is_aqp_exp(node):
    return (
        isinstance(node, exp.ErrorTarget)
...
class Query(object):
    ...
    def all_queries_in_expressions(self):
        ...
        for node, _, _ in self._expression.walk():
            if isinstance(node, exp.Select):
                ...
                if depth != 0:
                    if (
                        extracted_query.is_approx_agg_query
                        or extracted_query.is_approx_select_query
                    ):
                        raise Exception(
                            "AIDB does not support using approx query as a subquery"
                        )
    ...
    def udf_outputs_aliases(self):
        ...
        for node, _, _ in self._expression.walk(bfs=False):
            ...
            if (
                ...
                if (
                    isinstance(node.args["this"], exp.UserFunction)
                    and node.args["alias"].args["this"] is not None
                    and node.args["this"].args["this"]
                    in self.config.user_defined_functions
                ):
                    udf_output_alias_key = (
                        f"{node.args['this'].args['this']}__{alias_index}"
                    )
                    alias_index += 1
                    ...
                    if alias not in alias_to_udf_mapping:
                        ...
                        raise Exception(
                            "Duplicated alias is not allowed, please use another alias"
                        )
            elif (
                isinstance(node, exp.Aliases)
                and "expressions" in node.args
                and "this" in node.args
            ):
                if (
                    isinstance(node.args["this"], exp.UserFunction)
                    and len(node.args["expressions"]) != 0
                    and node.args["this"].args["this"]
                    in self.config.user_defined_functions
                ):
                    udf_output_alias_key = (
    ...
    def columns_in_query(self):
        ...
        """
        column_set = self._get_normalized_column_set(
            self.query_after_normalizing.get_expression()
        )
        return column_set
    ...
    def _get_sympify_form(self, node, predicate_count, predicate_mappings):
        if node is None:
            ...
            expression, predicate_count = self._get_sympify_form(
            ...
            isinstance(node, exp.GT)
            or isinstance(node, exp.LT)
            or isinstance(node, exp.GTE)
            or isinstance(node, exp.LTE)
            or isinstance(node, exp.EQ)
            or isinstance(node, exp.Like)
            or isinstance(node, exp.NEQ)
            or isinstance(node, exp.In)
            or isinstance(node, exp.Column)
        ):
    ...
    def _get_filtering_predicate_cnf_representation(
        self, cnf_expression, predicate_mappings
    ):
        if "&" not in str(cnf_expression):
            return [
                self._get_or_clause_representation(cnf_expression, predicate_mappings)
            ]
    ...
    def _replace_col_in_filter_predicate_with_root_col(self, expression):
        ...
        for node, _, _ in copied_expression.walk():
            ...
            if isinstance(node, exp.Column):
                ...
                originated_from = self.config.columns_to_root_column.get(
                    normalized_col_name, normalized_col_name
                )
    ...
    def _convert_logical_condition_to_cnf(self, logic_condition):
        ...
        sympy_representation, _ = self._get_sympify_form(
            logic_condition, 0, predicate_mappings
        )
        sympy_expression = sympify(sympy_representation)
        cnf_expression = to_cnf(sympy_expression)
        converted_logical_expression = self._get_filtering_predicate_cnf_representation(
    ...
    def filtering_predicates(self):
        ...
        if normalized_exp.find(exp.Where) is not None:
            ...
            filtering_predicates = self._convert_logical_condition_to_cnf(
                normalized_exp.find(exp.Where)
            )
            filtering_clauses = []
    ...
    def _get_table_of_column(self, col_name):
        ...
        if len(tables_of_column) == 0:
            raise Exception(
                f"Column - {col_name} is not present in any table or it not any alias of user defined function"
            )
        elif len(tables_of_column) > 1:
            raise Exception(
                f"Ambiguity in identifying column - {col_name}, it is present in multiple tables"
            )
        else:
    ...
    def query_after_normalizing(self):
        ...
        table_alias_to_name, column_alias_to_name = (
            self.table_and_column_aliases_in_query
        )
                ...
                if isinstance(node.args["this"], exp.Identifier):
                    ...
                    for table_name in self.tables_in_query:
                        for col_name, _ in self._tables[table_name].items():
                            ...
                            select_exp_list.append(
                                exp.Column(this=new_column, table=new_table)
                            )
                    copied_expression.set("expressions", select_exp_list)

        # remove alias

        copied_expression.set(
            "expressions",
            _remove_alias_in_expression(copied_expression.args["expressions"]),
        )
        copied_expression.args["from"].set(
            "expressions",
            _remove_alias_in_expression(
                copied_expression.args["from"].args["expressions"]
            ),
        )
    ...
    def inference_engines_required_for_filtering_predicates(self):
        ...
        for filtering_predicate in self.filtering_predicates:
            ...
            for or_connected_predicate in filtering_predicate:
                normalized_col_set = self._get_normalized_column_set(
                    or_connected_predicate
                )
    ...
    def error_target(self):
        ...
        return error_target / 100.0 if error_target else None
    ...
    def aggregation_type_list_in_query(self):
        ...
        for expression in select_exp:
            ...
            if isinstance(aggregate_expression, exp.Avg):
                ...
                raise Exception(
                    "AIDB only support approximation for Avg, Sum and Count query currently."
                )
    ...
    def is_valid_aqp_query(self):
        ...
        if not self.error_target or not self.confidence:
            raise Exception(
                "Aggregation query should contain error target and confidence"
            )
    ...
    def is_aqp_join_query(self):
        return (
            self._expression.find(exp.Join) is not None
            and self._expression.find(exp.UserFunction) is not None
        )

    @cached_property
    def recall_target(self):
        recall_target = self._get_keyword_arg(exp.RecallTarget)
        return recall_target / 100.0 if recall_target else None

    @cached_property
    def precision_target(self):
        precision_target = self._get_keyword_arg(exp.PrecisionTarget)
        return precision_target / 100.0 if precision_target else None
    ...
    def is_approx_select_query(self):
        if self.precision_target is not None:
            raise Exception(
                "AIDB has not support approx select query with precision target."
            )
    ...
    def convert_and_connected_fp_to_exp(and_connected_fp_list):
        ...
        def _build_tree(elements, operator):
            if len(elements) == 0:
                ...
                return operator(
                    this=elements[0], expression=_build_tree(elements[1:], operator)
                )
        ...
        for or_connected_fp in and_connected_fp_list:
            ...
            if new_or_connected_expression:
                new_or_connected_expression_list.append(
                    exp.Paren(this=new_or_connected_expression)
                )

        new_and_connected_expression = _build_tree(
            new_or_connected_expression_list, exp.And
        )
    ...
    def check_udf_query_validity(self):
        for expression in self._expression.find_all(exp.UserFunction):
            if isinstance(expression.parent, exp.UserFunction):
                if (
                    expression.parent.args["this"] in self.config.user_defined_functions
                    and expression.args["this"] in self.config.user_defined_functions
                ):
                    raise Exception(
                        "AIDB does not support nested user defined function currently"
                    )

        if self._expression.find(exp.AggFunc) is not None:
            if not self.is_aqp_join_query:
                raise Exception(
                    "AIDB does not support user defined function for exact aggregation query"
                )

        if self.is_approx_select_query or self.is_limit_query():
            raise Exception(
                "AIDB only support user defined function for exact query "
                "and approximate aggregation query currently."
            )

        if len(self.all_queries_in_expressions) > 1:
            raise Exception(
                "AIDB does not support user defined function with nested query currently"
            )
        ...
        for node, _, _ in expression.walk():
            ...
            if isinstance(node, exp.Column):
                ...
                if node.args["this"].args["this"] in alias_to_udf_mapping:
                    if (
                        alias_to_udf_mapping[node.args["this"].args["this"]].args[
                            "this"
                        ]
                        in self.config.user_defined_functions
                    ):
    ...
    def udf_query(self):
        ...
        * parsed_query (executed by normal db)
        ...
        class QueryModifier:
            ...
            def add_column_with_alias(
                self, node, is_select_col=False, predefined_alias=None
            ):
        ...
        for select_exp in expression.args["expressions"]:
            ...
            if (
                user_function
                and user_function.args["this"] in aidb_user_defined_functions
            ):
                _ = modified_query.add_udf(
                    user_function, udf_output_to_alias_mapping, is_select_col=True
                )
            else:
                ...
                if join_exp.args["on"] is not None:
                    ...
                    if (
                        ...
                        filter_predicates.extend(
                            self._convert_logical_condition_to_cnf(
                                join_exp.args["on"]
                            ).copy()
                        )
        ...
        if expression.find(exp.Where):
            filter_predicates.extend(
                self._convert_logical_condition_to_cnf(expression.find(exp.Where))
            )
        ...
        for or_connected in filter_predicates:
            ...
            for fp in or_connected:
                ...
                for node, _, key in fp_copy.walk(bfs=False):
                    if isinstance(node, exp.Expression) and self._check_in_subquery(
                        node
                    ):
                        continue
                    if isinstance(node, exp.UserFunction):
                        if node.args["this"] in aidb_user_defined_functions:
                            output_alias = modified_query.add_udf(
                                node.copy(), udf_output_to_alias_mapping
                            )
                            ...
                            converted_fp = exp.Column(
                                this=exp.Identifier(this=output_alias[0])
                            )
                            ...
                            node.parent.set(
                                key, modified_query.col_index_mapping[node_copy]
                            )
                        ...
                        modified_query.add_column_with_alias(node_copy)
                        node.args["this"].set(
                            "this", modified_query.col_index_mapping[node_copy]
                        )
                        node.set("table", None)
                new_or_connected_fp.append(fp_copy)
            filter_predicates_in_dataframe.append(new_or_connected_fp)

        new_where_condition = self.convert_and_connected_fp_to_exp(
            filter_predicates_in_sql
        )
