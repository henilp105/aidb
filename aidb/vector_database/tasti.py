...
class Tasti(TastiConfig):
    ...
    def set_vector_ids(self, vector_ids: pd.DataFrame):
        ...
        self.embeddings = self.vector_database.get_embeddings_by_id(
            self.index_name, self.vector_ids.values.reshape(1, -1)[0], reload=True
        )
    ...
    def get_topk_representatives_for_all(self, top_k: int = 5) -> pd.DataFrame:
        ...
        if self.reps is None:
            self._FPF()
        topk_reps, topk_dists = self.vector_database.execute(
            self.rep_index_name, self.embeddings, self.reps, top_k
        )
        topk_reps = self.vector_ids.iloc[np.concatenate(topk_reps)].values.reshape(
            -1, top_k
        )
    ...
    def get_topk_representatives_for_new_embeddings(
        self, new_vector_ids: pd.DataFrame, top_k: int = 5
    ) -> pd.DataFrame:
        """
        ...
        new_embeddings = self.vector_database.get_embeddings_by_id(
            self.index_name, new_vector_ids.values.reshape(1, -1)[0], reload=True
        )
        topk_reps, topk_dists = self.vector_database.query_by_embedding(
            self.rep_index_name, new_embeddings, top_k
        )
        topk_reps = self.vector_ids.iloc[np.concatenate(topk_reps)].values.reshape(
            -1, top_k
        )
