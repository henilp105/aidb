...
class WeaviateVectorDatabase(VectorDatabase):
    def __init__(self, weaviate_auth: WeaviateAuth):
        ...
        auth_client_secret = self._get_auth_secret(
            weaviate_auth.username, weaviate_auth.pwd, weaviate_auth.api_key
        )
        self.weaviate_client = weaviate.Client(
            url=weaviate_auth.url, auth_client_secret=auth_client_secret
        )
    ...
    def _get_auth_secret(
        ...
        if api_key:
            ...
            raise Exception(
                "Please provide api or username and password to connect Weaviate"
            )
    ...
    def create_index(
        ...
        if similarity not in ["cosine", "dot", "l2-squared"]:
            raise Exception(
                "Similarity function must be one of euclidean, cosine and dotproduct"
            )

        if index_name in self.index_list:
            raise Exception(
                f"Index {index_name} already exists, please use another name"
            )
    ...
    def get_embeddings_by_id(
        self, index_name: str, ids: np.ndarray, reload=False
    ) -> np.ndarray:
        """
        ...
        for id in ids:
            ...
            fetch_response = self.weaviate_client.data_object.get(
                uuid=uuid, with_vector=True, class_name=index_name
            )
    ...
    def query_by_embedding(
        ...
        for alias, query_emb in zip(alias_list, query_emb_list):
            new_query = self.weaviate_client.query.get(
                class_name=index_name, properties=properties
            )
    ...
    def execute(
        self, index_name: str, embeddings: np.ndarray, reps: np.ndarray, top_k: int = 5
    ) -> (np.ndarray, np.ndarray):
        """
        ...
        topk_reps, topk_dists = self.query_by_embedding(
            index_name, embeddings, top_k=top_k
        )
