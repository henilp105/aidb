...
class FaissVectorDatabase(VectorDatabase):
    ...
    def create_index(
        ...
        if similarity in ("dot_product", "cosine"):
            ...
            raise Exception(
                "Similarity function must be one of l2, cosine and dot_product"
            )
    ...
    def insert_data(self, index_name: str, data: pd.DataFrame):
        ...
        if not connected_index.is_trained:
            raise Exception(
                f"FAISS index of type {connected_index} must be trained before adding vectors"
            )
    ...
    def query_by_embedding(
        self, index_name: str, query_embeddings: np.ndarray, top_k: int = 5
        ...
        """
        connected_index = self._connect_by_index(index_name)
        all_topk_dists, all_topk_reps = connected_index.search(
            query_embeddings.astype("float32"), top_k
        )

        return np.array(all_topk_reps).astype("int64"), np.array(all_topk_dists).astype(
            "float32"
        )
