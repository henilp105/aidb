...
def convert_response_to_output(
    response: Union[Dict, List], _response_keys_to_columns: Dict[Union[str, tuple], int]
) -> Dict:
            ...
            if isinstance(key, AIDBListType) and isinstance(response_copy, list):
                if idx != len(k) - 1:
                    ...
                    for r in response_copy:
                        new_key = k[(idx + 1) :]
                        current_response = convert_response_to_output(
                            r, {new_key: new_key}
                        )
                    ...
                    response_copy = (
                        new_response_copy if len(new_response_copy) > 0 else None
                    )
                    break
            elif (
                isinstance(key, int)
                and (isinstance(response_copy, list) and key < len(response_copy))
                or (isinstance(response_copy, dict) and key in response_copy)
            ) or (
                isinstance(key, str)
                and isinstance(response_copy, dict)
                and key in response_copy
            ):
...
class HTTPInferenceService(CachedInferenceService):
    def __init__(
        ...
        **kwargs,
        ...
        assert not input_columns_types or len(input_columns_types) == len(
        ...
        assert not output_columns_types or len(output_columns_types) == len(
        ...
        if self._input_columns_types is not None:
            for k, _type in zip(dict_input_keys, self._input_columns_types):
                if len(dict_input[k]) > 0:
                    assert isinstance(
                        dict_input[k][0], _type
                    ), f"Input column {k} must be of type {_type}"
            ...
            if isinstance(v, tuple):
                ...
                if aidb_list_count == 0:
                    ...
                    for i in range(num_rows):
                        key = tuple(
                            f"{i}" if isinstance(_k, AIDBListType) else f"{_k}"
                            for _k in v
                        )
                        if isinstance(v[0], AIDBListType):
                            key = (
                                "_",
                            ) + key  # all converted keys should start with AIDBListType
                            remove_ghost_key = True
                        key = self._separator.join(key)
                        request[key] = dict_input[k][i]
                else:
                    raise ValueError(
                        f"Cannot have more than 1 AIDBListType in columns_to_input_keys"
                    )
    ...
    def convert_response_to_output(self, response: Dict) -> pd.DataFrame:
        self._response_keys_to_columns = {
            k: i for i, k in enumerate(self._response_keys_to_columns)
        }
        ...
        if self._output_columns_types is not None:
            for k, _type in zip(output.keys(), self._output_columns_types):
                if len(output[k]) > 0:
                    assert isinstance(
                        output[k][0], _type
                    ), f"Output column {k} must be of type {_type}"
    ...
    def infer_batch(self, inputs: pd.DataFrame):
        ...
        if self._url.endswith("__join"):
            ...
            if len(response) != len(inputs):
                raise Exception(
                    "The length of the inference results should match that of the inputs."
                )
