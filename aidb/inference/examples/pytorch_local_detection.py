...
class PyTorchLocalObjectDetection(CachedInferenceService):
    def __init__(
        ...
        super().__init__(
            name=name, preferred_batch_size=batch_size, is_single=is_single
        )
    ...
    def infer_one(self, input: pd.Series) -> pd.DataFrame:
        ...
        output = self._model.predict_with_caption(
            image, self._caption, self._box_threshold
        )[0]
        output = [
            {
                "image": image[0],
    ...
    def infer_batch(self, inputs: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(0, len(images), self.preferred_batch_size):
            image_batch = (
                images[i : i + self.preferred_batch_size]
                if i + self.preferred_batch_size < len(images)
                else images[i:]
            )
            outputs = self._model.predict_with_caption(
                image_batch, self._caption, self._box_threshold
            )
            outputs = [
                {
                    "image": image,
                ...
                for image, output in zip(image_batch, outputs)
                for xyxy, conf in zip(output.xyxy, output.confidence)
            ]
