...
class CachedBoundInferenceService(BoundInferenceService):
    ...
    def __post_init__(self):
        self._cache_table_name = cache_table_name_from_inputs(
            self.service.name, self.binding.input_columns
        )
        ...
        def get_table(conn: sqlalchemy.engine.base.Connection):
            ...
            for column_name in self.binding.input_columns:
                ...
                if column.primary_key:
                    new_table_col_name = (
                        self.convert_normalized_col_name_to_cache_col_name(column_name)
                    )
                    ...
                    if fk_ref_table_name not in fk_constraints:
                        fk_constraints[fk_ref_table_name] = {
                            "cols": [],
                            "cols_refs": [],
                        }
                    ...
                    columns.append(
                        sqlalchemy.schema.Column(new_table_col_name, column.type)
                    )

            multi_table_fk_constraints = []
            for _, fk_cons in fk_constraints.items():
                multi_table_fk_constraints.append(
                    ForeignKeyConstraint(fk_cons["cols"], fk_cons["cols_refs"])
                )

            table = sqlalchemy.schema.Table(
                self._cache_table_name, metadata, *columns, *multi_table_fk_constraints
            )
        ...
        for table_name in output_tables:
            ...
            for col in self._tables[table_name].columns:
                for cache_col in self._cache_columns:
                    ...
                    if col.name == normal_name:
                        condition.append(
                            getattr(self._cache_table.c, cache_col.name)
            ...
            joined = sqlalchemy.join(
                joined, self._tables[table_name]._table, join_condition
            )
        self._result_query_stub = sqlalchemy.sql.select(
            output_cols_with_label
        ).select_from(joined)
    ...
    async def _insert_in_cache_table(self, inp_rows_df: pd.DataFrame, conn):
        inp_rows_df.columns = [
            self.convert_normalized_col_name_to_cache_col_name(col)
            for col in inp_rows_df.columns
        ]
        ...
        await conn.run_sync(
            lambda conn: inp_rows_df.to_sql(
                self._cache_table.name, conn, if_exists="append", index=False
            )
        )

    async def _insert_output_results_in_tables(
        self, output_data: List[pd.DataFrame], input_data: pd.DataFrame, conn
    ):
        if len(output_data) > 0:
            for input_col, output_col in self.copy_map.items():
                assert len(output_data) == len(
                    input_data
                ), "Each input row should have 1 corresponding output dataframe, even if it is empty"
            ...
            for idx, col in enumerate(self.binding.output_columns):
                inference_results.rename(
                    columns={inference_results.columns[idx]: col}, inplace=True
                )
            tables = self.get_tables(self.binding.output_columns)
            for table in tables:
                columns = [
                    col
                    for col in self.binding.output_columns
    ...
    async def _get_inputs_not_in_cache_table(self, inputs: pd.DataFrame, conn):
        ...
        cache_entries = await conn.run_sync(
            lambda conn: pd.read_sql_query(
                text(str(self._cache_query_stub.compile())), conn
            )
        )
        cache_entries = cache_entries.set_index(
            [col.name for col in self._cache_columns]
        )
        normalized_cache_cols = [
            self.convert_cache_column_name_to_normalized_column_name(col.name)
            for col in self._cache_columns
        ]
    ...
    async def infer(self, inputs: pd.DataFrame, return_inference_results=False):
        ...
        async with self._engine.begin() as conn:
            (
                inputs_not_in_cache,
                inputs_not_in_cache_primary_cols,
                inputs_in_cache_primary_df,
            ...
            input_batches = [
                inputs_not_in_cache.iloc[i : i + bs]
                for i in range(0, len(inputs_not_in_cache), bs)
            ]
            ...
            await self._insert_output_results_in_tables(
                records_to_insert_in_table, inputs_not_in_cache, conn
            )
        ...
        if return_inference_results:
            ...
            for _, inp_row in inputs_in_cache_primary_df.iterrows():
                key_tuple = tuple(
                    pandas_dtype_to_native_type(getattr(inp_row, col))
            ...
            if sampled_key_list:
                ...
                sql_columns = [
                    getattr(self._cache_table.c, col_name) for col_name in columns
                ]
                where_condition = tuple_(*sql_columns).in_(sampled_key_list)
                query = self._result_query_stub.where(where_condition)
                async with self._engine.begin() as conn:
                    cached_df = await conn.run_sync(
                        lambda conn: pd.read_sql(query, conn)
                    )
            ...
            merged_df = pd.merge(
                inputs, inference_df, left_on=left_on_cols, right_on=right_on_cols
            )
