...
def cache_table_name_from_inputs(service_name: str, columns: List[str]):
    ...
    hash_length = 10
    column_input_hash = hashlib.sha1(cache_table_postfix.encode()).hexdigest()[
        :hash_length
    ]
...
def table_name_for_rep_and_topk_and_blob_mapping(blob_tables: List[str]):
    ...
    # limit to 10 characters to avoid long names in the table names
    hash_length = 10
    table_input_hash = hashlib.sha1(blob_table_postfix.encode()).hexdigest()[
        :hash_length
    ]
    return (
        f"{REP_PREFIX}__{table_input_hash}",
        f"{TOPK_PREFIX}__{table_input_hash}",
