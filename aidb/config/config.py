...
from aidb.config.config_types import Column, Graph, Table
...
from aidb.utils.constants import BLOB_TABLE_NAMES_TABLE, CACHE_PREFIX, CONFIG_PREFIX
...
class Config:
    ...
    def column_by_service(self) -> Dict[str, BoundInferenceService]:
        ...
        column_service = dict()
        for bound_service in self.inference_bindings:
            actual_output_columns = set(bound_service.binding.output_columns) - set(
                bound_service.copy_map.values()
            )
            for output_col in actual_output_columns:
                if output_col in column_service:
                    raise Exception(
                        f"Column {output_col} is bound to multiple services"
                    )
                else:
    ...
    def _check_blob_table(self):
        ...
        for blob_table in self.blob_tables:
            ...
            primary_key_set = set(
                [f"{blob_table}.{k}" for k in self.tables[blob_table].primary_key]
            )
    ...
    def _check_foreign_key_refers_to_primary_key(self, input_tables, output_tables):
        ...
        for output_table in output_tables:
            ...
            for fk_col, refers_to in self.tables[output_table].foreign_keys.items():
                if fk_col in self.tables[output_table].primary_key:
                    if refers_to not in input_primary_key_columns:
                        raise Exception(
                            f"{output_table} primary key column {fk_col} is not in input tables"
                        )
            ...
            for pk_col in input_primary_key_columns:
                if (
                    pk_col not in out_foreign_key_columns
                    and pk_col not in out_primary_key_columns
                ):
                    ...
                    while current_pk_col in self.tables[current_table].foreign_keys:
                        current_pk_col = self.tables[current_table].foreign_keys[
                            current_pk_col
                        ]
                        current_table = current_pk_col.split(".")[0]
                        if (
                            current_pk_col in out_foreign_key_columns
                            or current_pk_col in out_primary_key_columns
                        ):
                            raise_exception = False
                            break
                    if raise_exception:
                        raise Exception(
                            f"Primary key column {pk_col} in input table is not refered by output table {output_table}"
                        )
    ...
    def check_inference_service_validity(self, bound_inference: BoundInferenceService):
        ...
        if bound_inference.service.name not in self.inference_services:
            raise Exception(
                f"Inference service {bound_inference.service.name} is not defined in config"
            )
        ...
        if not binding.input_columns or not binding.output_columns:
            raise Exception(
                f"Inference service {bound_inference.service.name} has no input columns or output columns"
            )
        ...
        for input_col, output_col in bound_inference.copy_map.items():
            if input_col not in self.columns:
                raise Exception(
                    f"Input column in copy map {input_col} doesn't exist in database"
                )
            if output_col not in self.columns:
                raise Exception(
                    f"Output column in copy map {output_col} doesn't exist in database"
                )
        ...
        if not nx.is_directed_acyclic_graph(
            self.table_graph
        ) or not nx.is_directed_acyclic_graph(self.inference_graph):
            raise Exception(
                f"Inference service {bound_inference.service.name} will result in cycle in relations"
            )
    ...
    def load_from_sqlalchemy(self, conn: sqlalchemy.engine.base.Connection):
        ...
        blob_metadata_table = sqlalchemy.Table(
            BLOB_TABLE_NAMES_TABLE, metadata, autoload=True, autoload_with=conn
        )
        try:
            ...
            raise ValueError(
                f"Could not find blob metadata table {BLOB_TABLE_NAMES_TABLE} or table is malformed"
            )
