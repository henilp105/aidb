...
from aidb_utilities.vector_database_setup.vector_database_setup import (
    VectorDatabaseSetup,
)
...
class AidbVectorDatabaseSetupTests(IsolatedAsyncioTestCase):
    ...
    async def test_faiss_set_up(self):
        ...
        vector_database = VectorDatabaseSetup(
            DB_URL, blob_table_name, vd_type, index_name, auth
        )
        await vector_database.setup()

        existing_vector_database = FaissVectorDatabase(auth)
        value = existing_vector_database.get_embeddings_by_id(
            index_name, ids=np.array(range(3)), reload=True
        )
        test_equality(value)
