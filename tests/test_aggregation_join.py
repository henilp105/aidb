...
async def inference(
    inference_service: CachedBoundInferenceService, input_df: pd.DataFrame
):
...
class AggregateJoinEngineTests(IsolatedAsyncioTestCase):
    def add_user_defined_function(self, aidb_engine):
        ...
        aidb_engine._config.add_user_defined_function(
            "match_inference", async_match_inference
        )
    ...
    def _equality_check(self, aidb_res, gt_res, error_target):
        ...
        logger.info(
            f"Error rate (%) for approximate aggregation query: {error_rate_list}"
        )
    ...
    async def test_agg_query(self):
        ...
        for db_url in db_url_list:
            ...
            for i in range(_NUMBER_OF_RUNS):
                ...
                register_inference_services(
                    aidb_engine, data_dir, preferred_batch_size=1024
                )
                k = 0
                for query_type, aidb_query, aggregate_query in queries:
                    logger.info(
                        f"Running query {aggregate_query} in ground truth database"
                    )
                    try:
