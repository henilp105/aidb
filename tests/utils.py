import logging
from tests.db_utils.db_setup import (
    create_db,
    setup_db,
    setup_config_tables,
...
async def setup_gt_and_aidb_engine(db_url, data_dir, tasti_index=None, port=8000):
    ...
    if dialect == "postgresql" or dialect == "mysql":
        ...
        raise Exception(
            "Unsupported database. We support mysql, sqlite and postgresql currently."
        )
    ...
    engine = Engine(f"{db_url}/{aidb_db_fname}", debug=False, tasti_index=tasti_index)
