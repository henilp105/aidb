...
class FullScanEngineUdfTests(IsolatedAsyncioTestCase):
    ...
    def add_user_defined_function(self, aidb_engine):
        ...
        def replace_color(inp_df):
            color_value = np.where(
                inp_df.iloc[:, 0] == inp_df.iloc[:, 1],
                inp_df.iloc[:, 2],
                inp_df.iloc[:, 0],
            )
        ...
        aidb_engine._config.add_user_defined_function(
        ...
        aidb_engine._config.add_user_defined_function(
            "objects_inference", async_objects_inference
        )
        aidb_engine._config.add_user_defined_function(
            "lights_inference", async_lights_inference
        )
        aidb_engine._config.add_user_defined_function(
            "colors_inference", async_colors_inference
        )
        aidb_engine._config.add_user_defined_function(
            "counts_inference", async_counts_inference
        )
    ...
    async def test_udf_sqlite(self):
        ...
        p.start()
        time.sleep(1)
        gt_engine, aidb_engine = await setup_gt_and_aidb_engine(
            SQLITE_URL, data_dir, port
        )
    ...
    async def test_udf_postgresql_and_mysql(self):
        ...
        for db_url in [POSTGRESQL_URL, MYSQL_URL]:
            ...
            try:
                async with gt_engine.begin() as conn:
                    await conn.execute(
                        text("DROP FUNCTION IF EXISTS database_multiply_function;")
                    )
                    await conn.execute(
                        text("DROP FUNCTION IF EXISTS database_add_function;")
                    )
            ...
            for query_type, aidb_query, exact_query in queries:
                ...
                logger.info(
                    f"Avg relative difference percentage between gt_res and aidb_res: {avg_diff}"
                )
