...
def run_server(data_dir: str, port=8000):
    ...
    for csv_fname in inference_csv_fnames:
        ...
        async def inference(inp: Request):
            ...
            inp_df = pd.DataFrame(
                {
                    col: [inp[col]] if not isinstance(inp[col], list) else inp[col]
            ...
            result_df = pd.merge(
                inp_df, df, how="left", on=name_to_input_cols[service_name]
            ).convert_dtypes()
        ...
        async def join_inference(inp: Request):
            ...
            output_to_input_mapping = {
                col.split(".")[1]: col for col in name_to_input_cols[service_name]
            }
            for col in df.columns:
                if col not in inp_df.columns:
                    if col.split(".")[1] in output_to_input_mapping:
                        service_output_cols.append(
                            output_to_input_mapping[col.split(".")[1]]
                        )
                    else:
