            ...
            for csv_fname in gt_csv_fnames:
                ...
                for column in df.columns:
                    ...
                    if column_info.refers_to is not None:
                        ...
                        fk_constraints[fk_ref_table_name]["cols"].append(
                            column_info.name
                        )
                ...
                _ = sqlalchemy.Table(
                    ...
                    *[
                        sqlalchemy.Column(
                            c_info.name,
                            c_info.dtype,
                            primary_key=c_info.is_primary_key,
                            autoincrement=False,
                        )
...
async def insert_data_in_tables(conn, data_dir: str, only_blob_data: bool):
    ...
    gt_csv_fnames = await conn.run_sync(
        get_insertion_order,
        gt_csv_fnames,
    )
    # Create tables
    for csv_fname in gt_csv_fnames:
        ...
        if (
            only_blob_data
            and not table_name.startswith("blobs")
            and not table_name.startswith("mapping_")
        ):
            continue
        df = pd.read_csv(csv_fname)
        for column in df.columns:
        ...
        if table_name.startswith("mapping"):
            _, _, blob_mapping_table_name = (
                table_name_for_rep_and_topk_and_blob_mapping(["blobs_00"])
            )
            table_name = blob_mapping_table_name

        await conn.run_sync(
            lambda conn: df.to_sql(table_name, conn, if_exists="append", index=False)
        )
...
async def setup_config_tables(conn):
    ...
    def get_blob_table_names_and_columns(conn):
        ...
        table_names = [
            table.name
            for table in metadata.sorted_tables
            if table.name.startswith("blob")
        ]
        # Get the columns for each table
        table_names_and_columns = {}
        for table_name in table_names:
            table = metadata.tables[table_name]
            table_names_and_columns[table_name] = [
    ...
    for table_name in blob_table_names:
        for column in columns[table_name]:
            ...
            await conn.execute(
                text(
                    f"INSERT INTO {BLOB_TABLE_NAMES_TABLE} VALUES (:table_name, :blob_key)"
