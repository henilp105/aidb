...
from aidb.vector_database.weaviate_vector_database import (
    WeaviateAuth,
...
class TastiTests:
    def __init__(
        ...
        self.user_database = self.simulate_user_providing_database(
            index_path, weaviate_auth
        )

        self.tasti = Tasti(
            index_name, self.user_database, nb_buckets, percent_fpf, seed
        )
    ...
    def generate_data(self, data_size, emb_size):
        ...
        data = pd.DataFrame(
            {
                "id": range(self.total_data, self.total_data + data_size),
                "values": embeddings.tolist(),
            }
        )
        vector_ids = pd.DataFrame(
            {VECTOR_ID_COLUMN: range(self.total_data, self.total_data + data_size)}
        )
        self.total_data += data_size
        return data, vector_ids

    def simulate_user_providing_database(
        self, index_path: Optional[str], weaviate_auth: Optional[WeaviateAuth]
    ):
        """
        ...
        if self.vd_type == VectorDatabaseType.FAISS.value:
            ...
            user_database.create_index(
                self.index_name, self.embedding_dim, recreate_index=True
            )
    ...
    def test(self):
        ...
        logger.info(
            f"The shape of cluster representative ids: {representative_vector_ids.shape}"
        )
        ...
        if self.vd_type == VectorDatabaseType.FAISS.value:
            for representative_id in list(representative_vector_ids):
                assert (
                    representative_id
                    in topk_representatives.loc[representative_id]["topk_reps"]
                )
        ...
        logger.info(
            self.tasti.get_topk_representatives_for_new_embeddings(new_vector_ids)
        )
        ...
        logger.info(
            f"The total number of cluster representatives is: {len(self.tasti.reps)}"
        )
...
if __name__ == "__main__":
    ...
    test(
        "faiss",
        VectorDatabaseType.FAISS.value,
        data_size=10000,
        embedding_dim=128,
        nb_buckets=1000,
        index_path="./",
    ...
    test(
        "chroma",
        VectorDatabaseType.CHROMA.value,
        data_size=10000,
        embedding_dim=128,
        nb_buckets=1000,
        index_path="./",
    ...
    test(
        "Weaviate",
        VectorDatabaseType.WEAVIATE.value,
        data_size=200,
        embedding_dim=128,
        nb_buckets=50,
        weaviate_auth=weaviate_auth,
