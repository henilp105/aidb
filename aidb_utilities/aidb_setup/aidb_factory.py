...
def get_tasti_config(tasti_config):
    ...
    try:
        user_vector_database = vector_database[vector_database_type](
            **vector_database_config["auth"]
        )
    except KeyError:
        raise ValueError(
            f"{vector_database_type} is not a supported type. We support FAISS, Chroma and Weaviate."
        )
    tasti_index = Tasti(
        vector_database=user_vector_database, **tasti_config.tasti_engine
    )

    selected_vector_id_df = None

    if (
        hasattr(tasti_config, "vector_id_csv")
        and tasti_config.vector_id_csv is not None
    ):
        selected_vector_id_df = pd.read_csv(tasti_config.vector_id_csv)
        if len(selected_vector_id_df.columns) != 1:
            raise Exception(
                "Vector id csv file should contain one column for vector id"
            )
    ...
    def from_config(config_path, verbose=False):
        ...
        for inference_engine in config.inference_engines:
            ...
            aidb_engine.bind_inference_service(
                service.name, InferenceBinding(input_col, output_col), copy_map, verbose
            )
        return aidb_engine
