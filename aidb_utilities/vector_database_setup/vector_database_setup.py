...
class VectorDatabaseSetup:
    ...
    def _get_embedding(self, vector_database_df: pd.DataFrame) -> pd.DataFrame:
        ...
        def generate_data(vector_database_df):
            ...
            data = pd.DataFrame(
                {
                    "id": vector_database_df[VECTOR_ID_COLUMN],
    ...
    def _create_vector_database_index(self, data: pd.DataFrame):
        ...
        try:
            ...
            raise ValueError(
                f"{self.vd_type} is not a supported database type. We support FAISS, Chroma and Weaviate."
            )

        if self.vd_type == "FAISS":
            embedding_length = len(data["values"].iloc[0])
            user_vector_database.create_index(
                self.index_name, embedding_length, recreate_index=True
            )
        else:
    ...
    async def setup(self, path_column: Optional[str] = None):
        ...
        async with self._sql_engine.begin() as conn:
            ...
            _, _, blob_mapping_table_name = (
                table_name_for_rep_and_topk_and_blob_mapping([self.blob_table_name])
            )
            await conn.run_sync(
                lambda conn: blob_mapping_table_df.to_sql(
                    blob_mapping_table_name, conn, index=False, if_exists="append"
                )
            )
