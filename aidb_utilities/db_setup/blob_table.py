...
class BaseTablesSetup(object):
    ...
    async def _create_table(self, table_name, table_columns):
        ...
        async with self._sql_engine.begin() as conn:
            ...
            _ = sqlalchemy.Table(
                ...
                metadata,
                *[
                    sqlalchemy.Column(c_name, c_type, primary_key=is_pk)
                    for c_name, c_type, is_pk in table_columns
                ],
            )
    ...
    async def _setup_blob_config_table(self, table_name, table_columns):
        ...
        await self._create_table(
            ...
            [
                ("table_name", sqlalchemy.String(20), True),
                ("blob_key", sqlalchemy.String(20), True),
            ],
        )
        async with self._sql_engine.begin() as conn:
            for c_name, _, is_pk in table_columns:
                try:
                    ...
                    print(
                        f"Skipping: Blob config table already have {table_name} and {c_name}"
                    )
    ...
    def insert_blob_meta_data(
        self,
        table_name: str,
        blob_data: Union[pd.DataFrame, List[Blob]],
        ...
        self.create_input_table(
            ...
            list(
                map(
                    lambda x: {
                        "name": x,
                        "dtype": blob_data[x].dtype,
                        "is_primary_key": x in primary_key_cols,
                    },
                    blob_data.columns,
                )
            ),
    ...
    def create_input_table(self, table_name: str, blob_table_columns: List[dict]):
        ...
        for column in blob_table_columns:
            ...
            table_columns.append(
                (column["name"], c_type, column.get("is_primary_key", False))
            )
